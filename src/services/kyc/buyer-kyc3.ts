...
import ProfileBuyerKYC3ValidateSchema from '@validator/profile.buyer.kyc3';
import { KYCService } from './abstract-kyc';
import { KYC } from '../api/kyc';
import { ApiConfiguration } from '../api/http/config';
import { ProfileBuyerKYC3Serializer } from '@serializer/profile.serializer';
...
export class Serializer extends KYCSerializer {
    constructor() {
        super();
        this.fields = ['document_type', 'document_url', 'identity_card', 'passport'];
    }
}

export class BuyerKYC3Service extends KYCService {
    ...
    validate(payload: any): void {
        const { error, values } = Joi.validate(payload, ProfileBuyerKYC3ValidateSchema);
        if (error !== null) {
            throw error;
        }
    }
    ...
    async updateKYC(kyc_account: string, data: any): Promise<any> {
        ...
        let form = [{ "level": ApiConfiguration.FORM_BUYER_ENGAGED, "content": data }];
    ...
    async call(userId: string, payload: any): Promise<any> {
        ...
        return await new ProfileBuyerKYC3Serializer().serialize(user);
    }
